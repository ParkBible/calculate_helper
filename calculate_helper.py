...
b1_2 = Button(wd, text="총액 계산", command=cal).grid(row=9, column=1)
...
def cal2():
    ...
    e2_2['text'] = "이전타임 + 총 금액 : " + str(sum(eval_List2) + sum(eval_List1))
...
Label(wd, text="마감시 계산").grid(row=20, column=0, pady=10)
...
for i in range(8):
    label_List3[i] = Label(wd, text=message_List3[i]).grid(row=21+i, column=0)
    entry_list3[i] = Entry(wd)
    entry_list3[i].insert(0, 0)
    label2_List3[i] = Label(wd, text="금액 : ")

for i in range(8):
    label2_List3[i].grid(row=21+i, column=3)
    entry_list3[i].grid(row=21+i, column=1)

a3 = Button(wd, text="초기화", command=init3).grid(row=29, column=0)
e3 = Label(wd, text="총 금액 : ")
b3_2 = Button(wd, text="총액 계산", command=cal3).grid(row=29, column=1)
e3.grid(row=29, column=3)
#수정 끝
